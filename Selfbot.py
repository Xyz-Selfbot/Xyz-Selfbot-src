...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Xyz Selfbot v{SELFBOT.__version__}] | Loading...')
...
{Fore.RED}                                          ###    ###    ###    #########
{Fore.RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                                          {Fore.BLUE}━ USERNAME      | {Xyz.user.name}#{Xyz.user.discriminator}
                                          {Fore.BLUE}━ ID            | {Xyz.user.id}
                                          {Fore.BLUE}━ PREFIX        | {prefix}
                                          {Fore.BLUE}━ NITRO         | {nitro}
                                          {Fore.BLUE}━ GiVEAWAY      | {giveaway}
    '''+Fore.RESET)
...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            Xyz.run(token, bot=False, reconnect=True)
            os.system(f'title (Xyz Selfbot) - Version {SELFBOT.__version__}')
...
Xyz = discord.Client()
Xyz = commands.Bot(
    description='Xyz Selfbot',
    command_prefix=prefix,
    self_bot=True
)
Xyz.remove_command('help') 
...
async def btc_status():
    ...
    await Xyz.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await Xyz.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{Xyz.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await Xyz.process_commands(message)
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Xyz Selfbot v{SELFBOT.__version__}] | Logged in as {Xyz.user.name}')
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'Xyz-Was-Here@Fuckyou.com';
...
@Xyz.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@Xyz.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Xyz.command(aliases=['pfp', 'avatar'])
...
@Xyz.command(name='1337-speak', aliases=['1337speak'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:      
             await Xyz.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Xyz.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await Xyz.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Xyz.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
@Xyz.command(aliases=['dong', 'penis'])
...
@Xyz.command(aliases=['changehypesquad'])
...
@Xyz.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@Xyz.command(aliases=['fakeconnection', 'spoofconnection'])
...
@Xyz.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await Xyz.create_guild(f'backup-{ctx.guild.name}')
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://Xyz.wtf",
            reason="https://Xyz-selfbot.github.io",
            icon=None,
            banner=None
        )  
    except:
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = Xyz.get_user(user.id)
    if ctx.author.id == Xyz.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@Xyz.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
@Xyz.command(aliases=['rainbow-role'])
...
@Xyz.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = Xyz.get_channel(int(channelid))
...
@Xyz.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in Xyz.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)   
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in Xyz.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )

@Xyz.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Xyz.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Xyz.user).map(lambda m: m):
        try:
           await message.delete()
        except:
            pass

@Xyz.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in Xyz.private_channels:
...
async def help(ctx): # b'\xfc'
    await ctx.message.delete()
    url = 'https://Xyz-selfbot.github.io/commands'
    r = requests.get(url)
    if r.status_code == 200:
        webbrowser.open(url)
    else:
        print('Page is currently under maintenance')    
...
async def stream(ctx, *, message): # b'\xfc'
    ...
    await Xyz.change_presence(activity=stream)    
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await Xyz.change_presence(activity=game)

@Xyz.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await Xyz.change_presence(
...
@Xyz.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in Xyz.guilds:
        await guild.ack()
...
@Xyz.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@Xyz.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
@Xyz.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()   
    btc_status.start()        

@Xyz.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)

@Xyz.command(aliases=['clearconsole', 'consoleclear'])
...
@Xyz.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
